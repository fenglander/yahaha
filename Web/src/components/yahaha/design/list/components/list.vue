...
<template>
  <div ref="container" v-loading="state.loading" class="table-list-comm">
    <list-tree-side v-if="treeData.show" :data="treeData" :currentNodeKey="state.currentNodeKey"
      @node-click="treeNodeClick" />
    <div class="table-list">
      <div v-if="searchData?.list?.length" class="table-search">
        <Transition name="autoHeight">
          <form-renderer v-show="!state.searchFormDown" ref="searchFormEl" :dict="listDict" :disabled="state.loading"
            :form-data="searchData" is-search requestUrl="" @btn-click="formBtnClick">
      ...
      <div class="control-btn">
        <div class="btn-group">
          <el-button v-bind="item" v-for="item in tableData.controlBtn" :key="item.key" @click="controlBtnClick(item)">
            {{ item.label }}
          </el-button>
          <slot name="controlBtn"></slot>
        </div>
        <div class="control-other">
          <el-button v-if="searchFormExpand" circle icon="Search" size="small" title="展开/收起筛选"
            @click="state.searchFormDown = !state.searchFormDown" />
          <el-popover v-if="columnsSetting" :width="80" placement="bottom-end" trigger="click" @hide="popoverHideClick"
            @show="popoverShowClick">
            <template #default>
              <el-checkbox-group v-model="state.columnsCheck">
                <el-checkbox v-for="item in tableData.columns" :key="item.prop || item.type"
                  :label="item.prop || item.type">{{ item.label }}
                </el-checkbox>
              </el-checkbox-group>
            </template>
            <template #reference>
              <el-button circle icon="SetUp" size="small" title="设置列显示隐藏" />
            </template>
          </el-popover>
        </div>
      </div>
      <div v-if="columnsFilter?.length" :class="{ 'hide-vertical-scroll': isFixedBottomScroll }" class="table-main">
        <el-table v-bind="tableData.tableProps" ref="table" :data="state.tableDataList"
          ...
          <template v-for="item in columnsFilter" :key="item.prop || item.label">
            <el-table-column v-bind="item" config="">
              ...
              <template v-else-if="
                ...
                " #default="scope">
                <el-tag :type="item.config?.tagList[scope.row[item.prop]]" effect="light">
              ...
              <template v-else-if="item.prop === '__control'" #default="scope">
                <div class="table-operate-btn">
                  <template v-for="(btn, index) in operateBtnList" :key="btn.key || index">
                    <template v-if="btn.key === 'del'">
                      <el-popconfirm v-if="getOperateVisible(btn, scope.row)" :title="btn.tip || '确定删除该记录?'"
                        cancel-button-text="取消" confirm-button-text="确认" @confirm="operateBtnClick(btn, scope.row)">
                        <template #reference>
                          <el-button v-bind="btn" link size="small" type="primary">{{ btn.label }}
                          </el-button>
                        </template>
                      </el-popconfirm>
                    </template>
                    <template v-else>
                      <el-button v-bind="btn" v-if="getOperateVisible(btn, scope.row)" link size="small" type="primary"
                        @click="operateBtnClick(btn, scope.row)">{{ btn.label }}
                      </el-button>
                    </template>
                  </template>
                  <el-dropdown v-if="tableData.config?.operateDropdown">
                    ...
                    <template #dropdown>
                      <el-dropdown-menu>
                        <template v-for="(m, index) in tableData.operateBtn.slice(1)" :key="index">
                          <el-dropdown-item v-if="getOperateVisible(m, scope.row)"
                            @click="operateBtnClick(m, scope.row)">{{ m.label }}
                          </el-dropdown-item>
                        </template>
                      </el-dropdown-menu>
                    </template>
      ...
      <div v-if="showPage && state.total > state.pageSize" class="table-page">
        <el-pagination v-model:currentPage="state.currentPage" v-model:page-size="state.pageSize"
          :page-sizes="[20, 30, 40, 50] as any" :total="state.total" layout="total, sizes, prev, pager, next, jumper"
          @size-change="handleSizeChange" @current-change="handleCurrentChange" />
      </div>
      <div v-if="state.tableScrollMargin" :style="{ height: state.tableScrollMargin }"></div>
    </div>
  </div>
</template>

<script lang="ts" setup>
import formRenderer from '../../form/components/formRenderer.vue'
import {
  computed,
  nextTick,
  onBeforeUnmount,
...
import Tooltip from '/@/components/yahaha/design/components/tooltip.vue'
import { getRequest } from '/@/api/visualDev';
import { ElMessage, ElMessageBox } from 'element-plus'
import type { FormData, TableData } from '../../types'
import { dateFormatting } from '/@/components/yahaha/design/utils/'
import ListTreeSide from './listTreeSide.vue'
import formatResult from '/@/components/yahaha/design/utils/formatResult'
